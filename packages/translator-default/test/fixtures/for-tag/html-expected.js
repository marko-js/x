...
_marko_template._ = _marko_renderer(
  function(input, out, _component, component, state) {
    ...
    for (const val of arr) {
      ...
      const _keyScope = `[${i}]`;
      out.w(
        `<div>${_marko_escapeXml(i)}: ${_marko_escapeXml(
          val
        )}</div><div></div><div></div>`
      );
    }
    ...
    for (const key in obj) {
      ...
      out.w(
        `<div>${_marko_escapeXml(key)}: ${_marko_escapeXml(
          val
        )}</div><div></div><div></div>`
      );
    }
    ...
    for (const val of list) {
      ...
      const _keyValue2 = `@${i}`;
      out.w(
        `<div>${_marko_escapeXml(list.length)}: ${_marko_escapeXml(val)}</div>`
      );
    }
